  ...
  <div id="app">
    <div class="main-box">
      <div class="dec-welcome-text">
        <h1>Welcome to Rollup Credit!</h1>
      </div>

      <div class="dec-information">
        <h3>You are interacting with the Credit Contract Factory: {{ CREDIT_FACTORY_CONTRACT_ADDRESS }} on the Zkync Network with ZK Rollups</h3>
...
const CREDIT_FACTORY_CONTRACT_ADDRESS = "0xb76eD02Dea1ba444609602BE5D587c4bFfd67153";
...
export default {
  ...
  methods: {
    initializeProviderAndSigner() {
      ...
      this.provider = new Provider('http://127.0.0.1:3050');
    ...
    async getOverrides() {
      if ("ETH".l1Address !== ETH_L1_ADDRESS) {
    ...
    async acceptCreditContract(credit) {
      ...
      try {
        const txHandle = await this.creditFactoryContract.createCreditContract(
            credit.index,
    ...
    async placeCreditAmountOnContract(credit) {
      ...
      try {
        ...
        const txHandle = await creditContract.placeCreditAmount(overrides);
        credit.txStatus = DecTransactionStatus.WAITING_FOR_COMMIT;
        await txHandle.wait();
    ...
    async getPlacedCreditAmount(credit) {
      ...
      try {
        const creditContract = initializeCreditContractWithAbi.call(this, credit);
        const txHandle = await creditContract.getPlacedCreditAmount();
        credit.txStatus = DecTransactionStatus.WAITING_FOR_COMMIT;
        await txHandle.wait();
        await this.updateMyCredits();
      } catch (e) {
    ...
    async payback(credit) {
      ...
      try {
        ...
        const txHandle = await creditContract.payback(overrides);
        credit.txStatus = DecTransactionStatus.WAITING_FOR_COMMIT;
        await txHandle.wait();
        await this.updateMyCredits();
      } catch (e) {
    ...
    async getPayedBackCredit(credit) {
      ...
      try {
        const creditContract = initializeCreditContractWithAbi.call(this, credit);
        const txHandle = await creditContract.getPayedBackCreditWei();
        credit.txStatus = DecTransactionStatus.WAITING_FOR_COMMIT;
        await txHandle.wait();
        await this.updateMyCredits();
      } catch (e) {
